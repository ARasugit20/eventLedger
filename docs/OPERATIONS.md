...
| `DLQ_STREAM` | `eventledger:stream:dlq` | Operational dead-letter mirror |
...
3. After `MAX_DELIVERY_ATTEMPTS` redeliveries, the worker atomically writes
   `dead_letter_events` and marks the event `failed` in Postgres.
4. The worker mirrors the record to the Redis DLQ stream, then ACKs the source message.
5. Permanent handler failures mark the event `failed` in Postgres and ACK the message.
6. Stale pending messages are reclaimed via `XAUTOCLAIM` each worker loop.

Postgres is the durable DLQ and analytics source of truth. `GET /analytics/dlq`
returns its count and oldest-item age. Replay is manual in v1 — inspect the durable
record and Redis mirror, then re-ingest if appropriate.
...
| Durable DLQ count increasing | Poison messages | Query `/analytics/dlq`, inspect durable rows and Redis mirror |
